...
class UnetrPPEncoder(nn.Module):
    # CHANGED: input_size adapted to 2D 512x512 patches and spatial_dims=2
    def __init__(self, input_size=[128 * 128, 64 * 64, 32 * 32, 16 * 16],dims=[32, 64, 128, 256],
                 proj_size =[64,64,64,32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=2, in_channels=1,
                 dropout=0.0, transformer_dropout_rate=0.1 ,**kwargs):
        super().__init__()

        self.downsample_layers = nn.ModuleList()  # stem and 3 intermediate downsampling conv layers
        stem_layer = nn.Sequential(
            # CHANGED: kernel_size and stride adapted to 2D
            get_conv_layer(spatial_dims, in_channels, dims[0], kernel_size=(4, 4), stride=(4, 4),
    ...
    def forward_features(self, x):
        ...
        for i in range(1, 4):
            ...
            if i == 3:  # Reshape the output of the last stage
                # CHANGED: 'd' (depth) dimension removed for 2D rearrange
